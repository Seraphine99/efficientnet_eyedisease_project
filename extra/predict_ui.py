...
def apply_heatmap(heatmap, original_img_pil, alpha=0.5): # Slightly higher alpha for better visibility
    original_img_np = np.array(original_img_pil)
    
    # Rescale heatmap and apply VIRIDIS (Purple to Yellow)
    ...
    # Resize and blend
    colored_heatmap = cv2.resize(colored_heatmap, (original_img_np.shape[1], original_img_np.shape[0]))
    superimposed_img = colored_heatmap * alpha + original_img_np * (1 - alpha)
...
def draw_spectrum(canvas, width, height):
    ...
    # Use VIRIDIS for the legend too
    spec_strip = cv2.applyColorMap(gradient, cv2.COLORMAP_VIRIDIS)
    spec_strip = cv2.cvtColor(spec_strip, cv2.COLOR_BGR2RGB) # Correct the color order
    
    spec_strip = cv2.resize(spec_strip, (width, height))
    spec_img = Image.fromarray(spec_strip)
    spec_tk = ImageTk.PhotoImage(spec_img)
    canvas.create_image(0, 0, anchor="nw", image=spec_tk)
    canvas.image = spec_tk
